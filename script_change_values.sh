...
for ((n=0;n<num_comp;n++)); do
  ...
  pathEnv=".${arr[$n,0]}.image.repository"  valueEnv="$ECR_REPOSITORY_DEV" yq -i 'eval(strenv(pathEnv)) = strenv(valueEnv)' ./chart-symphony/symphony/prueba_values.yaml
done
cat ./chart-symphony/symphony/prueba_values.yaml

pathEnv=".version"  valueEnv="$ACTUAL_SYMPHONY" yq -i 'eval(strenv(pathEnv)) = strenv(valueEnv)' ./chart-symphony/symphony/prueba_Chart.yaml
