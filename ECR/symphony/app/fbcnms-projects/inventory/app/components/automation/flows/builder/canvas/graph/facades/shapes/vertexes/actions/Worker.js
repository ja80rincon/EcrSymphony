...
export const ACTION_TYPE_ID = 'worker';
export const TYPE = getActionType(ACTION_TYPE_ID);
const FILL_COLOR = '#4856b0';

const CreateWorkerBaseClass = CreateActionBaseClass({
  actionName: ACTION_TYPE_ID,
  fillColor: FILL_COLOR,
  svgPath: '/inventory/static/svg/BlockActionCreateWorkorder.svg',
  defaultText: `${fbt('Worker', '')}`,
});

const TOTAL_SIZE = 72;
const PADDING = 5;

export default class CreateWorker extends CreateWorkerBaseClass
...
export function isCreateWorker(element: ?IVertexModel): boolean {
  if (element == null) {
    return false;
  }
