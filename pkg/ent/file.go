...
// was not loaded in eager-loading, or loaded but was not found.
func (e FileEdges) DocumentCategoryOrErr() (*DocumentCategory, error) {
	if e.loadedTypes[9] {
...
func (*File) fkValues() []interface{} {
	return []interface{}{
		...
		&sql.NullInt64{}, // document_category_files
...
func (f *File) assignValues(values ...interface{}) error {
	...
	if len(values) == len(file.ForeignKeys) {
		...
		if value, ok := values[1].(*sql.NullInt64); !ok {
			return fmt.Errorf("unexpected type %T for edge-field document_category_files", value)
		} else if value.Valid {
			f.document_category_files = new(int)
			*f.document_category_files = int(value.Int64)
		}
...
// QueryDocumentCategory queries the document_category edge of the File.
func (f *File) QueryDocumentCategory() *DocumentCategoryQuery {
	return (&FileClient{config: f.config}).QueryDocumentCategory(f)
}
