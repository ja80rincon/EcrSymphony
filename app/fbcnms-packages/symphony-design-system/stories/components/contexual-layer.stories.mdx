import BaseContexualLayer from '../../components/ContexualLayer/BaseContexualLayer';
...
```js
import BaseContexualLayer from '@symphony/design-system/components/ContexualLayer/BaseContexualLayer';
```
...
export const AboveContexualContainer = ({element}) => {
  ...
  return (
    <BaseContexualLayer context={element} position="above">
      <div>
        <Text variant="body2">Above the input</Text>
      </div>
    </BaseContexualLayer>
  );
};
